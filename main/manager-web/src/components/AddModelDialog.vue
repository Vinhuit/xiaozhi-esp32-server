  ...
  <el-dialog :visible="dialogVisible" @update:visible="handleVisibleChange" width="57%" center
    ...
    <div style="margin: 0 18px; text-align: left; padding: 10px; border-radius: 10px;">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
        <div style="font-size: 20px; font-weight: bold; color: #3d4566;">{{$t('addModelDialog.modelInfo')}}</div>
        <div style="display: flex; align-items: center; gap: 20px;">
          <div style="display: flex; align-items: center;">
            <span style="margin-right: 8px;">{{$t('addModelDialog.isEnabled')}}</span>
            <el-switch v-model="formData.isEnabled" class="custom-switch"></el-switch>
          </div>
          <div style="display: none; align-items: center;">
            <span style="margin-right: 8px;">{{$t('addModelDialog.isDefault')}}</span>
      ...
      <el-form :model="formData" label-width="100px" label-position="left" class="custom-form">
        <div style="display: flex; gap: 20px; margin-bottom: 0;">
          <el-form-item :label="$t('addModelDialog.modelName')" prop="modelName" style="flex: 1;">
            <el-input v-model="formData.modelName" :placeholder="$t('addModelDialog.modelNamePlaceholder')" class="custom-input-bg"></el-input>
          </el-form-item>
          <el-form-item :label="$t('addModelDialog.modelCode')" prop="modelCode" style="flex: 1;">
            <el-input v-model="formData.modelCode" :placeholder="$t('addModelDialog.modelCodePlaceholder')" class="custom-input-bg"></el-input>
          </el-form-item>
        </div>

        <div style="display: flex; gap: 20px; margin-bottom: 0;">
          <el-form-item :label="$t('addModelDialog.supplier')" prop="supplier" style="flex: 1;">
            <el-select v-model="formData.supplier" :placeholder="$t('addModelDialog.supplierPlaceholder')" class="custom-select custom-input-bg"
              style="width: 100%;" @focus="loadProviders" filterable>
              <el-option v-for="item in providers" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item :label="$t('addModelDialog.sortOrder')" prop="sortOrder" style="flex: 1;">
            <el-input v-model="formData.sort" type="number" :placeholder="$t('addModelDialog.sortOrderPlaceholder')" class="custom-input-bg"></el-input>
          </el-form-item>
        </div>


        <el-form-item :label="$t('addModelDialog.docLink')" prop="docLink" style="margin-bottom: 27px;">
          <el-input v-model="formData.docLink" :placeholder="$t('addModelDialog.docLinkPlaceholder')" class="custom-input-bg"></el-input>
        </el-form-item>

        <el-form-item :label="$t('addModelDialog.remark')" prop="remark" class="prop-remark">
          <el-input v-model="formData.remark" type="textarea" :rows="3" :placeholder="$t('addModelDialog.remarkPlaceholder')" :autosize="{ minRows: 3, maxRows: 5 }"
            class="custom-input-bg"></el-input>
        </el-form-item>
      </el-form>

      <div style="font-size: 20px; font-weight: bold; color: #3d4566; margin-bottom: 15px;">{{$t('addModelDialog.callInfo')}}</div>
      <div style="height: 2px; background: #e9e9e9; margin-bottom: 22px;"></div>
  ...
  methods: {
    ...
    confirm() {
      ...
      if (!this.formData.supplier) {
        this.$message.error(this.$t('addModelDialog.supplierRequired'));
        this.saving = false;
        return;
      }
