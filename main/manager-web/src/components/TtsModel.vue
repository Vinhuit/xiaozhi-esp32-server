    ...
    <div class="scroll-wrapper">
      <div class="table-container" ref="tableContainer" @scroll="handleScroll">
        <el-table v-loading="loading" :data="filteredTtsModels" style="width: 100%;" class="data-table"
          ...
          <el-table-column v-if="!showReferenceColumns" :label="$t('ttsModel.demo')" align="center" class-name="audio-column">
            <template slot-scope="scope">
              <div class="custom-audio-container">
                <el-input v-if="scope.row.editing" v-model="scope.row.voiceDemo" :placeholder="$t('ttsModel.demoPlaceholder')"
                  class="audio-input">
                </el-input>
          ...
          <el-table-column v-if="!showReferenceColumns" :label="$t('ttsModel.remark')" align="center">
            <template slot-scope="scope">
              <el-input v-if="scope.row.editing" type="textarea" :rows="1" autosize v-model="scope.row.remark"
                :placeholder="$t('ttsModel.remarkPlaceholder')" class="remark-input"></el-input>
              <span v-else>{{ scope.row.remark }}</span>
            </template>
          </el-table-column>
          <el-table-column v-if="showReferenceColumns" :label="$t('ttsModel.referenceAudio')" align="center">
            <template slot-scope="scope">
              <el-input v-if="scope.row.editing" v-model="scope.row.referenceAudio" :placeholder="$t('ttsModel.referenceAudioPlaceholder')"></el-input>
          ...
          <el-table-column v-if="showReferenceColumns" :label="$t('ttsModel.referenceText')" align="center">
            <template slot-scope="scope">
              <el-input v-if="scope.row.editing" v-model="scope.row.referenceText" :placeholder="$t('ttsModel.referenceTextPlaceholder')"></el-input>
  ...
  methods: {
    ...
    loadData() {
      ...
      Api.timbre.getVoiceList(params, (data) => {
        if (data.code === 0) {
          ...
          this.$message.error({
            message: data.msg || this.$t('ttsModel.getListFail'),
            showClose: true
          });
        }
        this.loading = false;
        ...
        this.$message.error({
          message: this.$t('ttsModel.loadFail'),
          showClose: true
        });
        this.loading = false;
      });
    ...
    saveEdit(row) {
      if (!row.voiceCode || !row.voiceName || !row.languageType) {
        this.$message.error({
          message: this.$t('ttsModel.voiceCodeNameLangRequired'),
          showClose: true
        });
        return;
      }
    ...
    handleResponse(res, row) {
      if (res.code === 0) {
        ...
        this.$message.error({
          message: res.msg || (row.id ? this.$t('ttsModel.updateFail') : this.$t('ttsModel.saveFail')),
          showClose: true
        });
      }
    },
    ...
    addNew() {
      ...
      if (hasEditing) {
        this.$message.warning(this.$t('ttsModel.completeEditFirst'));
        return;
      }
    ...
    deleteRow(row) {
      ...
      if (Array.isArray(row) && row.length === 0) {
        this.$message.warning(this.$t('ttsModel.selectToDelete'));
        return;
      }


      const voiceCount = voices.length;
      this.$confirm(this.$t('ttsModel.confirmDeleteSelected', {count: voiceCount}), this.$t('ttsModel.warning'), {
        confirmButtonText: this.$t('ttsModel.confirm'),
        cancelButtonText: this.$t('ttsModel.cancel'),
        type: "warning",
        distinguishCancelAndClose: true
      }).then(() => {
        const ids = voices.map(voice => voice.id);
        if (ids.some(id => !id)) {
          this.$message.error(this.$t('ttsModel.invalidId'));
          return;
        }
        ...
        Api.timbre.deleteVoice(ids, ({ data }) => {
          if (data.code === 0) {
            this.$message.success({
              message: this.$t('ttsModel.deleteSuccess', {count: voiceCount}),
              showClose: true
            });
            this.loadData(); // 刷新参数列表
          } else {
            this.$message.error({
              message: data.msg || this.$t('ttsModel.deleteFail'),
              showClose: true
            });
          }
        });
