<template>
  <el-dialog :title="title" :visible.sync="dialogVisible" :close-on-click-modal="false" @close="handleClose" @open="handleOpen">
    <el-form ref="form" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item :label="$t('firmwareDialog.firmwareFile')" prop="firmwarePath">
        <el-upload ref="upload" class="upload-demo" action="#" :http-request="handleUpload"
          ...
          <el-button size="small" type="primary">{{$t('firmwareDialog.upload')}}</el-button>
        ...
        <div class="hint-text">
          <span>{{$t('firmwareDialog.hint')}}</span>
        </div>
      </el-form-item>
    ...
    <div slot="footer" class="dialog-footer">
      <el-button @click="handleCancel">{{$t('firmwareDialog.cancel')}}</el-button>
      <el-button type="primary" @click="handleSubmit">{{$t('firmwareDialog.confirm')}}</el-button>
    </div>
  </el-dialog>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        firmwareName: [
          { required: true, message: this.$t('firmwareDialog.firmwareNameRequired'), trigger: 'blur' }
        ],
        type: [
          { required: true, message: this.$t('firmwareDialog.firmwareTypeRequired'), trigger: 'change' }
        ],
        version: [
          { required: true, message: this.$t('firmwareDialog.versionRequired'), trigger: 'blur' },
          { pattern: /^\d+\.\d+\.\d+$/, message: this.$t('firmwareDialog.versionFormatError'), trigger: 'blur' }
        ],
        firmwarePath: [
          { required: false, message: this.$t('firmwareDialog.firmwareFileRequired'), trigger: 'change' }
        ]
      }
    }
  },
  ...
  methods: {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (!this.form.id && !this.form.firmwarePath) {
            this.$message.error(this.$t('firmwareDialog.firmwareFileRequired'))
            return
          }
    ...
    beforeUpload(file) {
      ...
      if (!isValidType) {
        this.$message.error(this.$t('firmwareDialog.fileTypeError'))
        return false
      }
      if (!isValidSize) {
        this.$message.error(this.$t('firmwareDialog.fileSizeError'))
        return false
      }
      return true
    },
    handleUpload(options) {
      ...
      Api.ota.uploadFirmware(file, (res) => {
        ...
        if (res.code === 0) {
          ...
          this.$message.error(res.msg || this.$t('firmwareDialog.uploadFail'))
          this.isUploading = false
        }
      }, (progressEvent) => {
