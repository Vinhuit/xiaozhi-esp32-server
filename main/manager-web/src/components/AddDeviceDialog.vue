  ...
  <el-dialog :visible="visible" @close="handleClose" width="24%" center>
    ...
    <div style="margin: 22px 15px;">
      <div style="font-weight: 400;font-size: 14px;text-align: left;color: #3d4566;">
        ...
        <span style="font-size: 11px"> {{$t('addDeviceDialog.codeLabel')}}</span>
      </div>
      <div class="input-46" style="margin-top: 12px;">
        <el-input :placeholder="$t('addDeviceDialog.codePlaceholder')" v-model="deviceCode" @keyup.enter.native="confirm" />
      </div>
    </div>
    <div style="display: flex;margin: 15px 15px;gap: 7px;">
      <div class="dialog-btn" @click="confirm">
        {{$t('addDeviceDialog.confirm')}}
      </div>
      <div class="dialog-btn" style="background: #e6ebff;border: 1px solid #adbdff;color: #5778ff;" @click="cancel">
        {{$t('addDeviceDialog.cancel')}}
      </div>
    </div>
  </el-dialog>
</template>
  ...
  methods: {
    confirm() {
      if (!/^[\d]{6}$/.test(this.deviceCode)) {
        this.$message.error(this.$t('addDeviceDialog.codeError'));
        return;
      }
      this.loading = true;
        ...
        this.deviceCode, ({ data }) => {
          ...
          if (data.code === 0) {
            ...
            this.$message.success({
              message: this.$t('addDeviceDialog.success'),
              showClose: true
            });
            this.closeDialog();
          } else {
            this.$message.error({
              message: data.msg || this.$t('addDeviceDialog.fail'),
              showClose: true
            });
          }
        }
