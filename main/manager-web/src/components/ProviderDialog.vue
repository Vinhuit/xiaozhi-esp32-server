      ...
      <el-form :model="form" label-width="100px" :rules="rules" ref="form" class="custom-form">
        <div style="display: flex; gap: 20px; margin-bottom: 20px;">
          <el-form-item label="Category" prop="modelType" style="flex: 1;">
            <el-select v-model="form.modelType" placeholder="Please select category" class="custom-input-bg" style="width: 100%;">
        ...
        <div style="font-size: 20px; font-weight: bold; color: #3d4566; margin-bottom: 15px;">
          {{ $t('providerManagement.fieldConfig') }}  <!-- Field Configuration -->
...
export default {
  ...
  methods: {
    ...
    addField() {
      if (this.hasIncompleteFields) {
        this.$message.warning({
          message: 'Please complete the current field editing first',
          showClose: true
        });
        return;
      }
    ...
    batchRemoveFields() {
      ...
      this.$confirm(`Are you sure you want to delete the selected ${selectedFields.length} fields?`, 'Tip', {
        confirmButtonText: 'Confirm',
        cancelButtonText: 'Cancel',
        type: 'warning'
      }).then(() => {
