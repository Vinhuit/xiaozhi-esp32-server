<template>
  <el-dialog :visible.sync="visible" width="400px" center>
    <div
      ...
      {{$t('viewPasswordDialog.title')}}
    </div>
    <div style="height: 1px;background: #e8f0ff;" />
    <div style="margin: 22px 15px;">
      <div style="font-weight: 400;font-size: 14px;text-align: left;color: #3d4566;">
        <div style="color: red;display: inline-block;">*</div>
        {{$t('viewPasswordDialog.passwordLabel')}}
      </div>
    ...
    <div style="display: flex;margin: 15px 15px;gap: 7px;">
      <el-button class="dialog-btn" style="background: #e6ebff;border: 1px solid #adbdff;color: #5778ff;"
        @click="closeDialog">
        {{$t('viewPasswordDialog.close')}}
      </el-button>
      <el-button class="dialog-btn" style="background: #5778ff;color: white;" @click="copyPassword">
  ...
  methods: {
    ...
    copyPassword() {
      ...
      this.$message.success({
        message: this.$t('viewPasswordDialog.copied'),
        showClose: true
      })
    }
  }
