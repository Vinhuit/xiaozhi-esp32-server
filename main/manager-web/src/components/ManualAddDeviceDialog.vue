  ...
  <el-dialog :title="$t('manualAddDeviceDialog.title')" :visible="visible" @close="handleClose" width="30%" center>
    <div class="dialog-content">
      <el-form :model="deviceForm" :rules="rules" ref="deviceForm" label-width="100px">
        <el-form-item :label="$t('manualAddDeviceDialog.deviceModel')" prop="board">
          <el-select v-model="deviceForm.board" :placeholder="$t('manualAddDeviceDialog.deviceModelPlaceholder')" style="width: 100%">
            <el-option
...
export default {
  ...
  data() {
    ...
    const validateMac = (rule, value, callback) => {
      ...
      if (!value) {
        callback(new Error(this.$t('manualAddDeviceDialog.macAddressRequired')));
      } else if (!macRegex.test(value)) {
        callback(new Error(this.$t('manualAddDeviceDialog.macAddressFormat')));
      } else {
        callback();
      }
    };
    ...
    return {
      ...
      rules: {
        board: [
          { required: true, message: this.$t('manualAddDeviceDialog.deviceModelRequired'), trigger: 'change' }
        ],
        appVersion: [
          { required: true, message: this.$t('manualAddDeviceDialog.firmwareVersionRequired'), trigger: 'blur' }
        ],
        macAddress: [
  ...
  methods: {
    ...
    addDevice() {
      ...
      Api.device.manualAddDevice(params, ({ data }) => {
        if (data.code === 0) {
          this.$message.success(this.$t('manualAddDeviceDialog.success'));
          this.$emit('refresh');
          this.closeDialog();
        } else {
          this.$message.error(data.msg || this.$t('manualAddDeviceDialog.fail'));
        }
      });
    },
    closeDialog() {
