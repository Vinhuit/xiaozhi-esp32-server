  ...
  <el-dialog :visible.sync="dialogVisible" :close-on-click-modal="false" width="57%" center custom-class="custom-dialog" :show-close="false"
    ...
    <div style="margin: 0 18px; text-align: left; padding: 10px; border-radius: 10px;">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
        <div style="font-size: 20px; font-weight: bold; color: #3d4566;">{{$t('modelEditDialog.modelInfo')}}</div>
        <div style="display: flex; align-items: center; gap: 20px;">
          <div style="display: flex; align-items: center;">
            <span style="margin-right: 8px;">{{$t('modelEditDialog.isEnabled')}}</span>
            <el-switch v-model="form.isEnabled" :active-value="1" :inactive-value="0" class="custom-switch"></el-switch>
          </div>
          <div style="display: none; align-items: center;">
            <span style="margin-right: 8px;">{{$t('modelEditDialog.isDefault')}}</span>
      ...
      <el-form :model="form" ref="form" label-width="100px" label-position="left" class="custom-form">
        <div style="display: flex; gap: 20px; margin-bottom: 0;">
          <el-form-item :label="$t('modelEditDialog.modelName')" prop="name" style="flex: 1;">
            <el-input v-model="form.modelName" :placeholder="$t('modelEditDialog.modelNamePlaceholder')" class="custom-input-bg"></el-input>
          </el-form-item>
          <el-form-item :label="$t('modelEditDialog.modelCode')" prop="code" style="flex: 1;">
            <el-input v-model="form.modelCode" :placeholder="$t('modelEditDialog.modelCodePlaceholder')" class="custom-input-bg"></el-input>
          </el-form-item>
        </div>

        <div style="display: flex; gap: 20px; margin-bottom: 0;">
          <el-form-item :label="$t('modelEditDialog.supplier')" prop="supplier" style="flex: 1;">
            <el-select v-model="form.configJson.type" :placeholder="$t('modelEditDialog.supplierPlaceholder')" class="custom-select custom-input-bg"
              style="width: 100%;" @focus="loadProviders" filterable>
              <el-option v-for="item in providers" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item :label="$t('modelEditDialog.sortOrder')" prop="sort" style="flex: 1;">
            <el-input v-model.number="form.sort" type="number" :placeholder="$t('modelEditDialog.sortOrderPlaceholder')" class="custom-input-bg"></el-input>
          </el-form-item>
        </div>

        <el-form-item :label="$t('modelEditDialog.docLink')" prop="docUrl" style="margin-bottom: 27px;">
          <el-input v-model="form.docLink" :placeholder="$t('modelEditDialog.docLinkPlaceholder')" class="custom-input-bg"></el-input>
        </el-form-item>

        <el-form-item :label="$t('modelEditDialog.remark')" prop="remark" class="prop-remark">
          <el-input v-model="form.remark" type="textarea" :rows="3" :placeholder="$t('modelEditDialog.remarkPlaceholder')" :autosize="{ minRows: 3, maxRows: 5 }"
            class="custom-input-bg"></el-input>
        </el-form-item>
      </el-form>

      <div style="font-size: 20px; font-weight: bold; color: #3d4566; margin-bottom: 15px;">{{$t('modelEditDialog.callInfo')}}</div>
      <div style="height: 2px; background: #e9e9e9; margin-bottom: 22px;"></div>
          ...
          <div :key="rowIndex" style="display: flex; gap: 20px; margin-bottom: 0;">
            <el-form-item v-for="field in row" :key="field.prop" :label="field.label" :prop="field.prop"
              ...
              <template v-if="field.type === 'json-textarea'">
                <el-input v-model="fieldJsonMap[field.prop]" type="textarea" :rows="3" :placeholder="$t('modelEditDialog.jsonPlaceholder')"
  ...
  methods: {
    ...
    validateJson(value) {
      try {
        ...
        this.$message.error({
          message: this.$t('modelEditDialog.jsonDictError'),
          showClose: true
        });
        return null;
      } catch (e) {
        this.$message.error({
          message: this.$t('modelEditDialog.jsonFormatError'),
          showClose: true
        });
        return null;
      }
